...
type CheckoutError = { error: string; ready?: boolean };

function isRecord(v: unknown): v is Record<string, unknown> {
  return typeof v === "object" && v !== null;
}
function hasUrl(v: unknown): v is CheckoutSuccess {
  return isRecord(v) && typeof v.url === "string";
}
function getErrorMessage(v: unknown): string | null {
  return isRecord(v) && typeof (v as CheckoutError).error === "string"
    ? (v as CheckoutError).error
    : null;
}
...
export default function CheckoutPage() {
  ...
  async function startCheckout() {
    ...
    try {
      ...
      if (!res.ok || !hasUrl(data)) {
        setErr(getErrorMessage(data) ?? "Stripe is not ready yet.");
        setLoading(false);
        return;
      }
    ...
    <main className="mx-auto max-w-3xl px-5 py-12">
      ...
      <div className="mt-6 rounded-2xl border border-neutral-800 p-6 shadow-sm">
        ...
        <p className="mt-3 text-sm leading-6 text-neutral-300">
          Coaching is educational and not medical advice. Results are not guaranteed.
